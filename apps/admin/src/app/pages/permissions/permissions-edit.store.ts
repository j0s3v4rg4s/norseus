...
export const permissionsEditStore = signalStore(
  ...
  withMethods((store) => {
    ...
    const saveRole = async (roleName?: string) => {
      ...
      try {
        ...
        const newNameRole = roleName
          ...
          .join('_');
        const newPermissions = store.permissions().filter((item) => !item.id).map((item) => ({
          action: item.action,
    ...
    return { loadRole, addPermission, removePermission, deleteRole, saveRole };
  }),
);
